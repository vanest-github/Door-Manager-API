...
public class Handler : IRequestHandler<RequestModel, ServiceResult<bool>>
{
    ...
    public async Task<ServiceResult<bool>> Handle(RequestModel request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var doors = await this._doorAccessRepository.GetSelfAccessDoorsAsync(request.UserId, request.OfficeId, cancellationToken).ConfigureAwait(false);
            isValidAccess = doors.Contains(request.DoorId);

            var activity = ActivityLogType.SelfAccess.ToString();
